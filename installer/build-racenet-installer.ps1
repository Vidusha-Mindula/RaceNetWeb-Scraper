<#
    ...
    S3AccessKey/S3SecretKey/S3BucketName are baked into the installer (it seeds a default
    ...
    set RACENET_S3_ACCESS_KEY / RACENET_S3_SECRET_KEY / RACENET_S3_BUCKET_NAME as environment
    variables on the machine that builds the installer. Leaving all unset just ships an installer
    with blank defaults, same as before this existed.
#>
param(
    [string]$Version,
    ...
    [string]$S3BucketName = $env:RACENET_S3_BUCKET_NAME
)
...
& $isccPath "/DAppVersion=$Version" "/DS3AccessKey=$S3AccessKey" "/DS3SecretKey=$S3SecretKey" "/DS3BucketName=$S3BucketName" "/O$outputDir" $issScript
